...
void get_order(char *prompt);                                     //获取用户输入命令
...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        ordercount = 0;
        explain_order(buf, &ordercount, orderlist); //对输入的命令进行拆分
...
void get_order(char *prompt)
{
    /*该函数获得用户的指令，并实现TAB键补全,返回输入的指令指针*/
    free(buf);
    buf = readline(prompt);
        add_history(buf);    //该功能实现将用户输入的命令记录下来
...
void exec_order(int ordercount, char orderlist[100][256])
{
    ...
    char *file;
    pid_t pid;
    int flag_re=0; //用来记录是否在|后面的指令是否存在重定向操作
    /*将输入的命令取出*/
