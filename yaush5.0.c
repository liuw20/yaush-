...
{
    int ordercount = 3;
    char orderlist[100][256] = {"sleep", "5", "&"};
...
void exec_order(int ordercount, char orderlist[100][256])
{
    ...
    {
        wait(pid, NULL, 0);//如果没有后台执行命令，则等待子进程再退出
    }
}
...
int find_command(char *command)
{
    DIR *dp;
    struct dirent *dirp;
    char *path[] = {"./", "/bin", "/usr/bin", "/usr/local/sbin", "/usr/local/bin", "/usr/sbin", "/sbin", "/usr/games", "/usr/local/games", "/snap/bin", NULL};
